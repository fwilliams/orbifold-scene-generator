...
def gen_incompleteness_xml(xml_filename, tx_list, render_depth=False, use_bidir=False):
    ...
    if render_depth:
        ...
        if component_format:
            assert len(component_format) == 1, "Multiple componentFormats wtf..."
            film.remove(component_format[0])
...
def gen_incompleteness_xml_incremental(xml_filename, tx_list, xf, render_depth=False, use_bidir=False):

    # xf.write_declaration()
    # generate an element (the root element)
    with xf.element('scene', version="0.5.0"):
...
def gen_scene_xml_incremental(xml_filename, tx_list, xf):
