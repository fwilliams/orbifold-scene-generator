...
class FriezeReflectionGroup(object):
    def __init__(self, height, up, mirror1_ctr, mirror2_ctr, flag_ceiling, flag_floor):
        ...
        # The bottom vertices of the tfds, they are on the plane y = 0, so the tfds are generated by the ceiling mirrors.
        # These are the 2D vertices surrounding the translational fundamental domain on the floor
        tfdv = [b - right_proj*width*0.5, b + right_proj*width*0.5,
                b + right_proj*width*0.5 + m1n*width*2.0,
                b - right_proj * width * 0.5 + m1n*width*2.0]

        # If there is a floor mirror, move tfd vertices downward to make the tfds generated by the floor mirror.
        # If there is both a ceiling and a floor mirror, we use the same translational fundamental domain as if there
        # were only a ceiling mirror.
        if self._flag_floor:
